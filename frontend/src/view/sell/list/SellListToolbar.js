...
class SellToolbar extends Component {
  state = {
    ...
    return (
      <ToolbarWrapper>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Button
            component={Link}
            to="/audit-logs?entityNames=sell"
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: sellSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: sellSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(SellToolbar);
