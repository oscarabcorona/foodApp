import listActions from 'modules/bar/list/barListActions';
...
const actions = {
  ...
  doDestroy: (id) => async (dispatch) => {
    try {
      ...
      await BarService.destroyAll([id]);

      dispatch({
        type: actions.DESTROY_SUCCESS,
      });

      Message.success(i18n('entities.bar.destroy.success'));
  ...
  doDestroyAll: (ids) => async (dispatch) => {
    try {
      ...
      await BarService.destroyAll(ids);
      ...
      Message.success(
        i18n('entities.bar.destroyAll.success'),
      );
