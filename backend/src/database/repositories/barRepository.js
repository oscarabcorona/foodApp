...
/**
 * Handles database operations for the Bar.
 */
class BarRepository {
  /**
   * Creates the Bar.
  ...
  async create(data, options) {
    ...
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(
          `${new Bar().collectionName}/${record.id}`,
        ),
      record,
      options,
    );
  ...
  /**
   * Counts the number of Bars based on the filter.
   *
   * @param {Object} filter
   */
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Bar().collectionName);
  ...
  /**
   * Finds the Bar and its relations.
   *
   * @param {string} id
   */
  async findById(id) {
  ...
  /**
   * Finds the Bars based on the query.
  ...
  /**
   * Lists the Bars to populate the autocomplete.
   * customize the query.
   *
   * @param {Object} search
   * @param {number} limit
